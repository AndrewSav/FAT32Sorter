...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	while ((choise=menu(fatFileSystem.getCurrentDriveLetter())) != EXIT)
	{
		_tprintf(_T("\n\n"));
		switch (choise)
		{
			...
			case (SORT):
			{
				if (areYouSureMsg(_T("Are you sure you want to sort the entire files' table (backup will be saved) [y/n] ? ")))
				{			
	...
	//udini.loadFilesTable(_T("d:\\dirs.dat"));

	//udini.dumpFatsTable(_T("D:\\"));
	//udini.dumpFilesTable(_T("D:\\dirs.dat"));
	
	//udini.exportFoldersList(_T("D:\\FilesListBefore.txt"));
	//udini.exportFoldersList(_T("D:\\FilesListAfter.txt"));

	//udini.initFDT();
	//udini.dumpFilesTable(_T("D:\\dirsAfter.dat"));
	_CrtDumpMemoryLeaks();
